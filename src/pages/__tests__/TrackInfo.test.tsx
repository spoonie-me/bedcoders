...
describe('TrackInfo credential block', () => {
  it('prices a track that clears the depth bar', () => {
    ...
    expect(screen.queryByText(/no credential yet/i)).not.toBeInTheDocument();
  });
  ...
  it.skipIf(heldBack.length === 0)('shows no price at all for a track held back for depth', () => {
    ...
    expect(screen.getByText(/no credential yet/i)).toBeInTheDocument();
    // No price is attached to *this* track: the headline price line and the
    // pay-what-you-can offer that goes with it are both gone. (The other
    // tracks' pricing is described elsewhere on the site — that's the point.)
    expect(screen.queryByText(/no subscription, no renewal, ever/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/can't afford €69/i)).not.toBeInTheDocument();
  });
  ...
  it.skipIf(heldBack.length === 0)(
    ...
    () => {
      ...
      expect(screen.getByText(new RegExp(`needs at least ${remaining} more lesson`, 'i'))).toBeInTheDocument();
      expect(screen.getByText(/free to read right now/i)).toBeInTheDocument();
    },
  );
});
